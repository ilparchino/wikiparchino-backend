...
def create_user(client: httpx.Client, token: str, username: str = "utente") -> dict:
...
def test_self_safety_password_reset_and_session_revocation(client: httpx.Client) -> None:
    ...
    assert client.put(
        ...
        headers=headers(admin_token),
        json={"new_password": "password-nuova-sicura"},
    ).status_code == 204
    assert client.get("/api/me", headers=headers(old_token)).status_code == 401
    new_token = login(client, "reset", "password-nuova-sicura")
