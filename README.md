...
Groups represent Italian UI "Cerchie" and are regular pullable entities with rarity, media, search, and activity history. Group membership with people and epochs is many-to-many. Membership changes are submitted independently through the group relationship endpoints; reciprocal person and epoch routes are read-only.

Collection search endpoints accept `q` and an optional `limit` from 1 to 50, and return lightweight `id`, `title`, and `subtitle` results. Relationship collections accept server-side `page`, `page_size`, `q`, `sort`, `order`, and their documented typed filters, returning the standard page envelope with lightweight related-entity summaries. Candidate routes exclude entities that are already linked. Relationship writes use atomic `PATCH` changesets with explicit additions, metadata updates, and removals; unchanged changesets do not touch attribution or activity. Person-event links are written from events, person-group links are written from groups, and person-place links may be changed from either side. Place and epoch events are queried through `/events?place_id=...` and `/events?epoch_id=...`.
